defmodule Tilex.Web.PostController do
  ...
  def show(conn, _) do
    ...
    canonical_post = Application.get_env(:tilex, :canonical_domain) <> Tilex.Web.Router.Helpers.post_path(conn, :show, post)
  ...
  defp extract_slug(conn, _) do
    case extracted_slug(conn.params["titled_slug"]) do
      ...
      :error ->
        conn
        |> put_status(404)
        |> render(Tilex.Web.ErrorView, "404.html")
        |> halt()
    end
  end
